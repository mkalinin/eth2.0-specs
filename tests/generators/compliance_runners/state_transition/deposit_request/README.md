...
output: the appended `PendingDeposit` equals
`PendingDeposit(pubkey, withdrawal_credentials, amount, signature, slot=pre.slot)`,
the queue grew by exactly one, **`deposit_requests_start_index` is unchanged**
(the gloas modification), and `post == spec re-execution`.
