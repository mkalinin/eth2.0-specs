...
A third instance of the shared pipeline ([`../aspects/`](../aspects),
[`../aspect_coverage.py`](../aspect_coverage.py)), reusing two aspects from the
other handlers.
...
- **Real BLS** deposit signatures (valid / wrong-key), so
  `IGNORED_BAD_SIGNATURE` depends on genuine verification.
